<section class="card"><div class="section-head section-head-split"><div><h2>Assigned Students</h2><p class="muted">Students assigned to your company, including those still waiting for document forwarding.</p></div><input class="table-search table-search-wide" placeholder="Search students..."></div><div class="table-wrap"><table class="data-table"><thead><tr><th data-sort>Name</th><th data-sort>Student No.</th><th>Course/Year</th><th>Schedule</th><th>Pre-Deployment</th><th>Details</th></tr></thead><tbody><?php foreach ($students as $s): ?><?php $scheduleStart = $s['official_start_date'] ?? $s['start_date'] ?? null; $scheduleEnd = $s['projected_end_date'] ?? $s['end_date'] ?? null; $scheduleLabel = ($scheduleStart && $scheduleEnd) ? ($scheduleStart . ' to ' . $scheduleEnd) : 'Awaiting orientation completion'; $predeploymentLabel = ucwords(str_replace('_', ' ', $s['predeployment_status'] ?? '')); ?><tr data-detail-type="partner-student" data-detail-name="<?= e($s['student_name']) ?>" data-detail-email="<?= e($s['student_email']) ?>" data-detail-student-no="<?= e($s['student_no']) ?>" data-detail-course="<?= e($s['course']) ?>" data-detail-year="<?= e($s['year_level']) ?>" data-detail-schedule="<?= e($scheduleLabel) ?>" data-detail-predeployment="<?= e($predeploymentLabel) ?>"><td><?= e($s['student_name']) ?><br><small><?= e($s['student_email']) ?></small></td><td><?= e($s['student_no']) ?></td><td><?= e($s['course'] . ' ' . $s['year_level']) ?></td><td><?= e($scheduleLabel) ?></td><td><span class="badge <?= e($s['predeployment_status']) ?>"><?= e($predeploymentLabel) ?></span><br><small class="muted"><?= e($s['status']) ?></small></td><td><a class="btn btn-small" href="{{ route('partner.dashboard') }}?enrollment=<?= (int)$s['id'] ?>">Open</a></td></tr><?php endforeach; ?></tbody></table></div><div class="pagination"></div></section>
